...
const rangeFilter = (from, to) => {
  goods.filter((item) => from <= item.price && item.price <= to);
}


const minPriceReducer = () => {
  goods.reduce((accum, item) => (item.price < accum ? item.price : accum), Infinity)
};

const maxPriceReducer = () => {
  goods.reduce((accum, item) => (item.price > accum ? item.price : accum), 0)
};
